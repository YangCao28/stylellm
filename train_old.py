...
def setup_training(config: Config):
    ...
    if tokenizer.mask_token and len(tokenizer) > model.model.config.vocab_size:
...
def main():
    ...
    if args.eval_only:
        ...
        model = StyleAlignmentModel(
            model_name_or_path=config.training.output_dir,
            device="cuda:0"
        )
